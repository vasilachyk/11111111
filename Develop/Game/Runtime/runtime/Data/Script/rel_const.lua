...
-- DOF
CONST_DOF_FOCUS_DIST		= 150;			-- 카메라에서 포커스 위치의 거리
CONST_DOF_FOCUS_RANGE		= 60000;		-- 포커스되는 범위 (60,000)
CONST_DOF_COC_SIZE			= 4;				-- 착란원(circle of confusion, 흐려지는 아웃 포커싱 부분) 크기

-- 캐릭터 선택 화면의 DOF
CONST_CHAR_DOF_FOCUS_DIST		= 150;			-- 카메라에서 포커스 위치의 거리
CONST_CHAR_DOF_FOCUS_RANGE		= 20000;		-- 포커스되는 범위
CONST_CHAR_DOF_COC_SIZE			= 4;			-- 착란원(circle of confusion, 흐려지는 아웃 포커싱 부분) 크기
...
-- 모델 기본 메쉬
CONST_SEX_MALE					= false;		-- 기본 플레이어 성별 - const.lua에서 덮어쓰면 성별을 바꿀 수 있음

-- 기본 장비 세트
CONST_DEFAULT_EQUIPMENTS_SET0 = "9712,9713,9714,9715"; -- 최초 착용하는 의상
CONST_DEFAULT_EQUIPMENTS_SET1 = "9712,9713,9714,9715"; -- 최초 착용하는 의상2 
CONST_DEFAULT_EQUIPMENTS_SET2 = "9202,9203,9205";
CONST_DEFAULT_EQUIPMENTS_SET3 = "2902,2903,2904,2905,9000006,9000007";		-- 미리보기 디펜더
CONST_DEFAULT_EQUIPMENTS_SET4 = "3062,3063,3064,3065,123";				-- 미리보기 버서커
CONST_DEFAULT_EQUIPMENTS_SET5 = "9000072,9000073,9000074,9000075,9000076";						-- 미리보기 클레릭
CONST_DEFAULT_EQUIPMENTS_SET6 = "64043201,64043401,64043501,64010306";				-- 미리보기 소서러
CONST_DEFAULT_EQUIPMENTS_SET7 = "9000092,9000093,9000094,9000095,9000096";				-- 미리보기 어새신
...
-- RealSpace3
CONST_USING_COMPILED_SHADER = true;		-- 컴파일된 쉐이더 파일이 있으면 컴파일된 쉐이더 파일을 읽을지 여부
CONST_RESOURCE_LOADING_LOG = false;		-- 리소스 읽는 시간을 로그에 남길지 여부
...
CONST_SENSOR_EFFECT_NAME = "테스트";
CONST_FIELDMAP_OPACITY	= 0.5;		-- 필드맵 영역표시 OPACITY

CONST_DEBUG_EXECUTE_UNITTEST	= false;
CONST_DEBUG_LOG = false;

PLAYER_18_OVER = false;						-- 연령제한 (false : 18세 미만, true: 18세 이상)
...
-- DEBUG에서만 실행됨
CONST_USE_NET_DEBUGGER			= true;			-- NetDebugger 사용 여부
CONST_USE_PHYSX_REMOTE_DEBUGGER		= false;			-- Physx Remote Debugger 사용 여부
CONST_MAKE_COMMAND_RESULT	= false;		-- CCommandResultTable의 설명을 result.xml로 뽑는다.
CONST_LOG_COMMAND 			= false;		-- 이 값이 true이면 mlog_command.txt로 커맨드 로그가 남는다.
CONST_AUTO_SAVE_REPLAY		= false;		-- 게임시작하면 자동으로 리플레이 저장
CONST_COMMAND_PROFILER_AUTO_SAVE = false;	-- 게임 종료시 커맨드 프로파일러 로그를 자동으로 저장할지 여부
CONST_DEBUG_SHOW_INTERPOLATION_TARGET_INFO_NETPLAYER = false;   -- NetPlayer 서버로 받은 위치값 보여주기(보간 처리하는 패킷들...)
CONST_PLAY_ANALYZER		= true;		-- 게임시작하면 자동으로 리플레이 저장
CONST_LOAD_STRINGS = true; -- 디버그로 빌드할 때 속도를 높이려면 스트링을 읽지 마세요
CONST_DUMMY_CLIENT_LOG = false;		-- 더미 클라이언트에서 로그를 찍을지 여부
CONST_DUMMY_CLIENT_COMMAND_HISTORY_SIZE = 20;		-- 더미 클라이언트 커맨드 히스토리 크기
CONST_DUMMY_CLIENT_COMMAND_HISTORY_IGNORE_CMD_0 = 0; -- 더미 클라이언트 커맨드 히스토리에서 무시할 커맨드ID
-- ~ CONST_DUMMY_CLIENT_COMMAND_HISTORY_IGNORE_CMD_9 까지 지정할 수 있습니다.
...
-- 데칼
DECALEFFECT_DEPTH_VALUE = 100.0;		-- 1미터

-- 충돌 설정
CONST_COLLISION_CAPSULE_RADIUS_OFFSET = 20.0;			-- 충돌 캡슐 반지름 추가 값
CONST_COLLISION_TERRAIN_FALLING_START_TIME = 0.1;		-- 낙하 후 낙하 애니메이션 시작 시간...

-- 인터랙션 대상 외곽선 설정
CONST_INTERACTION_TARGET_EDGE_COLOR_R = 221;			-- 외곽선 색깔 R
CONST_INTERACTION_TARGET_EDGE_COLOR_G = 219;			-- 외곽선 색깔 G
CONST_INTERACTION_TARGET_EDGE_COLOR_B = 25;				-- 외곽선 색깔 B
CONST_INTERACTION_TARGET_EDGE_COLOR_A = 255;			-- 외곽선 색깔 A
CONST_INTERACTION_TARGET_EDGE_COLOR_POWER = 1.8;		-- 외곽선 색깔 쌔기
CONST_INTERACTION_TARGET_EDGE_BLUR_FACTOR = 0.8;		-- 외곽선 번짐 정도(두께로 생각하시면 됩니다. 색깔 쌔기로 해서 진하게 보이게 조절하시면 됩니다.)
...
CONST_INTERACTION_BPART_TARGET_EDGE_COLOR_R = 255;			-- BPART 외곽선 색깔 R
CONST_INTERACTION_BPART_TARGET_EDGE_COLOR_G = 0;			-- BPART 외곽선 색깔 G
CONST_INTERACTION_BPART_TARGET_EDGE_COLOR_B = 0;			-- BPART 외곽선 색깔 B
CONST_INTERACTION_BPART_TARGET_EDGE_COLOR_A = 255;			-- BPART 외곽선 색깔 A
CONST_BPART_GAIN_BUFF_EFFECT_NAME = "bparts_start";			-- BPART 얻을때 나오는 이펙트
CONST_BPART_LOST_BUFF_EFFECT_NAME = "bparts_end";			-- BPART 사라질때 나오는 이펙트
CONST_BPART_LOOTABLE_EFFECT_NAME = "bparts_loot";			-- BPART 루팅 이펙트
CONST_BPART_LOOTABLE_EFFECT_MIN_SIZE = 100;					-- BPART 루팅 이펙트 최소 사이즈(cm 단위)
CONST_BPART_LOOTABLE_EFFECT_MAX_SIZE = 400;					-- BPART 루팅 이펙트 최대 사이즈
CONST_BPART_LOOTABLE_EFFECT_SCALE_RATIO = 1.0;				-- BPART 루팅 스케일 적용 비율

-- Locale
CONST_IGNORE_NON_TRANSLATION_MARK = true;			-- 미번역 마크(#)를 무시해서 읽을지 여부(한국어만)

-- Help
CONST_INGAME_TIP_MAX_LEVEL = 5;			-- 게임중에 팁을 보여주는 최대 레벨
CONST_INGAME_TIP_TICK = 300;				-- 게임중에 팁 나오는 간격(초) 테스트용으로 180초로 해 놓음


-- 효과음
CONST_SOUND_DODGE_EFFECT_NAME = "";					-- 빗맞음
CONST_SOUND_RIPOSTE_EFFECT_NAME = "";						-- 되받아치기
CONST_SOUND_RESIST_PERFECT_EFFECT_NAME = "";				-- 되받아치기
CONST_SOUND_RESIST_PARTIAL_EFFECT_NAME = "";				-- 저항
CONST_SOUND_DRAIN_EFFECT_NAME = "";							-- 흡수
CONST_SOUND_IMMUNE_EFFECT_NAME = "pc_avoid";				-- 무효
CONST_SOUND_MISS_EFFECT_NAME = "";						-- 빗맞음
CONST_SOUND_AVOID_EFFECT_NAME = "pc_avoid";				-- 회피

-- 크로스 헤어 반경 설정
CONST_TARGET_CROSS_HAIR_SIZE = 1.0;						-- 기본값은 1입니다.
CONST_TARGET_ALWAYS_CHECK_TARGET_DISTANCE = 1500.0;		-- 기본값은 2미터입니다. 크로스헤어가 타겟을 검사하는 범위
CONST_TARGET_RADIUS_CORRECTION = 3.0;					-- 기본값은 x1배 입니다. 크로스헤어가 타겟을 검사하는 반경
CONST_CROSSHAIR_POS_HEIGHT_RATE = 0.42;							-- 크로스 헤어 위치 높이값
...
-- LowHP 경고창 HP 설정
CONST_LOWHP_PERCENTAGE = 20.0; -- 퍼센티지 값입니다. 이값보다 작아지면 LowHP 경고창 생성.

-- 연속기 탤런트 UI 위치 조절  -- 화면 중심에서의 위치입니다. X값이 증가할수록 오른쪽, Y값이 증가할수록 아래쪽으로 치우치게 됩니다.(픽셀단위)  
...
--
CONST_LIMIT_MAX_LEVEL = 40;			-- 스킬트리 레벨 제한

--
MAX_ENCHANT_GRADE = 9;						-- 최대 강화 등급
MAX_ELEMENT_ENCHANT_GRADE = 6;				-- 최대 속성 강화 등급
ELEMENT_ENCHANT_EFFECT_CHANGE = false;		-- 속성 강화시 이펙트 교체 적용 여부
...
-- Post Move
CONST_POST_MOVE_TICK = 0.3;					-- 이동 패킷 보내는 시간 Tick
CONST_POST_POS_SYNC_TICK = 0.2;				-- 서버랑 위치 동기화를 맞출 시간 Tick
CONST_POST_STOP_POS_SYNC_LENGTH = 500.0;	-- Stop시 위치 보정할 거리 이 크기만큼 차이 났을 때만 Stop 패킷으로 보정한다.
CONST_POST_MOVE_SEND_DALAY		= 0;		-- 100 - 0.1초

-- Talent Failed Message Delay Tiem
CONST_TALENT_RESULT_MSG_DELAY_TIME = 1000;		-- 탈렌트 사용 실패 메시지 출력 딜레이 타임 ( 1000 = 1초 )

-- GAME_DELIBERATION			-- 심의 관련 작업 적용
CONST_GAME_DELIBERATION						= false;
CONST_GAME_DELIBERATION_LOADING_DELAY		= 3000;		-- 3초
CONST_GAME_DELIBERATION_MSG_CYCLE			= 3600000;	-- 1시간( 1000 = 1초 )
CONST_GAME_DELIBERATION_LOGO_DELAY			= 3000;		-- 심의 마크 유지 시간( 3초 )

-- 필드 로딩 타임아웃
CONST_FIELD_LOADING_TIMEOUT					= 5;		-- 5분

-- 월드 로딩 체크타임(이시간이 지나면 지형로딩완료를 99%로 체크해서 넘긴다)
CONST_WORLD_LOADING_CHECK_TIME					= 1.5;		-- 1분30초

-- 부활 아이템
CONST_REBIRTH_ITEMS = "31502,31500,31501,1300000,2010200,3010200,1800302,1300000,4000000,18032190,18020012";		-- MAX 10개
...
-- QPVP 자리비움 강제 이동 시간(자리비움 시작했을시부터)
CONST_QUESTPVP_AFK_RETURN_TO_INN = 300;



-- 이벤트 로딩 이미지
CONST_EVENT_ENABLE = false;   -- 로딩 이미지에 이벤트 이미지를 보여줄것인지 셋팅하는 변수
CONST_EVENT_IMAGE_FULLSCREEN = false;   -- 이미지만 풀화면으로 보여줄것인지 셋팅하는 변수
CONST_EVENT_IMAGE_NUMBER = 99998;   -- 이미지 번호, 파일은  Data/Interface/game/resource/zoneillust <- 여기에 있어야 하고 필드이미지와 동일하게 5자리 숫자로 파일명 작성
CONST_EVENT_TITLE = "EVENT_TITLE_"..CONST_EVENT_IMAGE_NUMBER -- 이거 건들지 마세요. CONST_EVENT_IMAGE_NUMBER만 설정하시면 됩니다.
CONST_EVENT_DESC = "EVENT_DESE_"..CONST_EVENT_IMAGE_NUMBER -- 이거 건들지 마세요. Lang의 타이틀과 설명은 이미지 번호를 따라감.

--CONST_EVENT_TITLE = "{COLOR r=255 g=245 b=104} Happy Halloween Event ♡{/COLOR}"; -- 이벤트 제목을 기술한다.
--CONST_EVENT_DESC = "{COLOR r=249 g=173 b=129}'Treat or Trick'{/COLOR}{CR}할로윈 사탕을 주세요! 축제용 호박을 주세요!{CR}이벤트 기간에만 즐길 수 있는{COLOR r=244 g=154 b=193}절호의 찬스!!{/COLOR}{CR}모두 모아서 멋진 기념품으로 교환할 준비 되셨나요?{CR}{COLOR r=249 g=173 b=129}지옥마 이벤트도 진행하고 있으니, 고스트라이더와의 멋진 한판 승부도 즐겨보세요!{/COLOR}{CR}{CR}{COLOR r=109 g=207 b=246}장소 : 인젠 자유항{/COLOR}{CR}{COLOR r=124 g=197 b=118}You are Invited to HALLOWEEN PARTY~{/COLOR}"; -- 이벤트 설명을 기술한다.


--CONST_EVENT_IMAGE_NUMBER = 99995;   -- 이미지 번호, 파일은  Data/Interface/game/resource/zoneillust <- 여기에 있어야 하고 필드이미지와 동일하게 5자리 숫자로 파일명 작성
--CONST_EVENT_TITLE = "{COLOR r=255 g=245 b=104} 로자린드의 선물!{/COLOR}"; -- 이벤트 제목을 기술한다.
--CONST_EVENT_DESC = "부활절을 맞아 {COLOR r=249 g=173 b=129}특별한 선물{/COLOR}을 준비했어요.{CR}그런데 어쩌죠? 몬스터들이 제가 준비한 선물을 모두 가져가 버렸어요.{CR}몬스터들을 혼내주시면 {COLOR r=249 g=173 b=129}달걀 세트{/COLOR}를 얻으실 수 있을 거에요.{CR}아... 돌려주지는 않으셔도 되요. 이 모든 것은 여러분을 위해 준비한 것이니까요.{CR}{CR}{COLOR r=109 g=207 b=246}장소 : 모든 지역{/COLOR}"; -- 이벤트 설명을 기술한다.

--CONST_EVENT_IMAGE_NUMBER = 99996;
--CONST_EVENT_TITLE = "{COLOR r=255 g=245 b=104}어쌔신, 육체적 능력이 극한에 이른 자들.{/COLOR}"; -- 이벤트 제목을 기술한다.
--CONST_EVENT_DESC = "인젠 독립전쟁, 렌델 왕국의 건설... 대륙을 뒤흔든 역사적 사건의 종결.{CR}아직도 회자되는 빛나는 영웅들의 이면에는 항상 나 어둠의 조력자, {COLOR r=141 g=198 b=63}'어쌔신'{/COLOR}이 있었다.{CR}{CR}{COLOR r=244 g=154 b=193}※{/COLOR}{COLOR r=161 g=134 b=190}어쌔신 업데이트 기념, {/COLOR}{COLOR r=249 g=173 b=129}버닝타임 이벤트!{/COLOR}{CR}     {COLOR r=109 g=207 b=246}시간 : 1월22일(화) ~ 2월26일(화)  오후 12시 ~ 오후 2시, 오후 8시 ~ 오후 10시{CR}     버프효과 : 경험치 획득양 증가, 주요 능력치 증가, 생명력과 정신력 회복양 증가"; 

-- Idle 변경 시간 및 확률
CONST_RIDER_IDLE_CHANGE_TIME			= 10.0;		-- 몇초 단위로...
CONST_RIDER_IDLE_CHANGE_RATE_PERCENT	= 20;			-- 몇 퍼센트 확률로...

CONST_PLAYER_IDLE_CHANGE_TIME			= 10.0;		-- 몇초 단위로...
CONST_PLAYER_IDLE_CHANGE_RATE_PERCENT	= 20;			-- 몇 퍼센트 확률로...
...
{
	"{C1} = {COLOR r=111 g=220 b=89} 강조문구 표시",
	"{C2} = {COLOR r=255 g=0 b=10} 경고문구 표시",
};